...
// ----- Helpers -----
async function ensureUserDoc(uid: string, profile?: any) {
...
function monthsToMs(m: number) {
  return 1000 * 60 * 60 * 24 * 30 * m; // approx months
}

function addDaysMs(days: number) { return 1000 * 60 * 60 * 24 * days; }
...
export const checkAccess = functions.https.onCall(async (data, context) => {
  ...
  if (!u.exists) throw new functions.https.HttpsError('not-found', 'User missing');
...
export const createOrder = functions.https.onCall(async (data, context) => {
  ...
  const plan = (data?.plan || 'monthly') as string;
  ...
  if (!amount) throw new functions.https.HttpsError('invalid-argument', 'Invalid plan');
...
app.post('/razorpay/webhook', async (req, res) => {
  try {
    ...
    if (signature !== expected) return res.status(401).json({ ok: false, error: 'invalid signature' });
...
export const verifyIntegrity = functions.https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Sign-in required');
  const token = data?.integrityToken as string;
  const packageName = cfg?.play?.package || '';
  if (!token || !packageName) throw new functions.https.HttpsError('invalid-argument', 'Missing token/package');

  try {
    // Exchange for access token via default credentials
    const auth = await admin.credential.applicationDefault().getAccessToken();
    const resp = await fetch(`https://playintegrity.googleapis.com/v1/packageNames/${packageName}:decodeIntegrityToken`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${auth.access_token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ integrityToken: token }),
    });
    if (!resp.ok) throw new Error(`Integrity API ${resp.status}`);
  const body: any = await resp.json();

    // Basic checks (deviceIntegrity, appIntegrity, accountIntegrity)
  const verdicts: string[] = body?.deviceIntegrity?.deviceRecognitionVerdict || [];
  const appLic = body?.appLicensingVerdict || 'UNKNOWN';
    ...
    if (!passed) throw new Error('integrity failed');
...
export const expireLicenses = functions.pubsub.schedule('every 24 hours').onRun(async () => {
...
async function sendEmail(to: string, subject: string, text: string) {
  ...
  if (sg) {
    try {
      ...
      if (!r.ok) console.error('SendGrid send failed', await r.text());
      return;
...
export const auth_requestEmailOtp = functions.https.onCall(async (data, context) => {
...
export const auth_verifyEmailOtp = functions.https.onCall(async (data, context) => {
  const email = (data?.email || '').toString().trim().toLowerCase();
  const code = (data?.code || '').toString().trim();
  if (!email || !code) throw new functions.https.HttpsError('invalid-argument', 'Email and code required');

  let user: admin.auth.UserRecord | null = null;
  try { user = await admin.auth().getUserByEmail(email); } catch {}
  if (!user) throw new functions.https.HttpsError('not-found', 'User not found');

  const otpRef = db.collection('users').doc(user.uid).collection('auth_otp').doc('email');
  const snap = await otpRef.get();
  if (!snap.exists) throw new functions.https.HttpsError('failed-precondition', 'No code requested');
  ...
  if ((d.attempts || 0) >= 5) throw new functions.https.HttpsError('resource-exhausted', 'Too many attempts');

  const ok = d.code_hash === hashOtp(code, email);
  await otpRef.set({ attempts: (d.attempts || 0) + 1 }, { merge: true });
  if (!ok) throw new functions.https.HttpsError('permission-denied', 'Invalid code');
